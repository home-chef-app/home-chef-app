...
function init(){
    ...
    echo $AWS_REGION
    echo "test1"
    terraform init -backend-config="bucket=${TERRAFORM_BUCKET}" -backend-config="key=${tf_state_key}" -backend-config="region=${AWS_REGION}" -backend-config="access_key=${aws_access_key}" -backend-config="secret_key=${aws_secret_access_key}"
    echo "test"
    printf "\n***********************************************************************************\n\n"
